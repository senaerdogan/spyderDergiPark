...
def searchAndDownload(topic=None, searchPhrase=None, maxArticle=None, headless=False, language="türkçe", path=None):
    if (topic is None) and (searchPhrase is None):
        ...
        driver, current_url = searchByTopicAndPhrase(topic, searchPhrase, maxArticle, headless, language)
        download(driver, maxArticle, language, current_url, path)
    elif topic is not None:
        driver, current_url = searchByTopic(topic, maxArticle, headless, language)
        download(driver, maxArticle, language, current_url, path)
    elif searchPhrase is not None:
        driver, current_url = searchByPhrase(searchPhrase, maxArticle, headless, language)
        download(driver, maxArticle, language, current_url, path)
